...
class SQL_Database:
    ...
    def Insert(self, table, data): # DONE
        print(f"[SERVER][SQL-DATABASE] (Insert) Has been applied:[{table} :: {data}]")
        # Extract:
    ...
    def Update(self, table, data, where=None): # DONE
        print(f"[SERVER][SQL-DATABASE] (Update) Has been applied:[{table} :: {data} :: {where}]")
        # Set
        S = 'SET'; params = []
        for key in data:
    ...
    def Delete(self, table, where=None): # DONE
        print(f"[SERVER][SQL-DATABASE] (Delete) Has been applied:[{table } :: {where}]")
        # Extract:
        W = ''; params = []
        if where:
            W = 'WHERE'
    ...
    def Check(self, table, where=None, fetch=None, columns=['*']): # DONE
        print(f"[SERVER][SQL-DATABASE] (Check) Has been applied:[{table} :: {where} :: {fetch}]")
        # Extrect:
        W = ''; params = []
        if where:
            W ='WHERE'
        ...
        cur = self.db.cursor()
        cur.execute(f"""SELECT {columns[1:-1].replace('"', '').replace("'", '')} FROM {table}\n{W}\n{L};""", tuple(params))
        self.db.commit()
        if fetch: response = cur.fetchall(); cur.close(); return response 
        else: response = bool(cur.fetchone()); cur.close(); return response
